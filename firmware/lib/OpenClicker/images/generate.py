...
def GenerateImageHeaderFile(fp, image_path: Path, output_dict: dict):
    ...
    for i, pixel in enumerate(pixels):
        ...
        if i != len(pixels) - 1:
            fp.write(",")
    fp.write("\n};\n\n")
    struct_name = f"Graphics_{image_tag}"
    ...
    try:
        ...
        print(image_path.stem, raw_unicodes)
        raise 
...
def GenerateAllImagesHeaderFile(fp: TextIOWrapper, output_dict: dict):
    ...
    for category, images in output_dict["images"].items():
        for image in images:
            ...
            items.append(f"    const RemoteGraphic_t* {category.upper()}_{image['unicode'][0]} = &{image['struct_name']};")
    fp.write("\n")
