/**
 AUTOGENERATED ON 2024-06-15 13:03:38.301023
...
char* presenter_message_to_json(const uint8_t* message, const uint32_t message_size, char* from_mac, char* to_mac) {
    ...
    if ((message[0] == RemoteHeartBeat)) {
        ...
        int size_needed = snprintf(NULL, 0, "{\"from\":\"%s\",\"to\":\"%s\",\"msg\":{\"id\":26,\"battery_level\":%u,\"state_name\":\"%s\"}}",from_mac,to_mac,msg->battery_level,msg->state_name) + 1;
        char* json = (char*)malloc(size_needed);
        if (json == NULL) return NULL;
        bzero(json, size_needed);
        snprintf(json, size_needed, "{\"from\":\"%s\",\"to\":\"%s\",\"msg\":{\"id\":26,\"battery_level\":%u,\"state_name\":\"%s\"}}",from_mac,to_mac,msg->battery_level,msg->state_name);
        return json;
    }
...
char* presenter_data_to_json(const uint8_t* message, const uint32_t message_size) {
    ...
    if ((message[0] == RemoteHeartBeat)) {
        PresenterProtocolRemoteHeartBeat_t* msg = (PresenterProtocolRemoteHeartBeat_t*)message;
        int size_needed = snprintf(NULL, 0, "{\"id\":26,\"battery_level\":%u,\"state_name\":\"%s\"}",msg->battery_level,msg->state_name) + 1;
        char* json = (char*)malloc(size_needed);
        if (json == NULL) return NULL;
        bzero(json, size_needed);
        snprintf(json, size_needed, "{\"id\":26,\"battery_level\":%u,\"state_name\":\"%s\"}",msg->battery_level,msg->state_name);
        return json;
    }
